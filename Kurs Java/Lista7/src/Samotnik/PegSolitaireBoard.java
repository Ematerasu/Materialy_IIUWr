...
public class PegSolitaireBoard {
    ...
    public void MovePiece(int xCord, int yCord, int xDest, int yDest) {
        if(IsMoveViable(xCord, yCord, xDest, yDest) && turn == Turn.SELECTION) {
    ...
    public boolean IsGameOver() {
        for(int i = 0; i < 7; i++) {
            for(int j = 0; j < 7; j++) {
                if(board[i][j].getIsPiece() && board[i][j].getIsGone()) {
                    if (IsMoveViable(board[i][j].xCord, board[i][j].yCord, board[i][j].xCord + 2, board[i][j].yCord))
                        return false;
                    if (IsMoveViable(board[i][j].xCord, board[i][j].yCord, board[i][j].xCord - 2, board[i][j].yCord))
                        return false;
                    if (IsMoveViable(board[i][j].xCord, board[i][j].yCord, board[i][j].xCord, board[i][j].yCord + 2))
                        return false;
                    if (IsMoveViable(board[i][j].xCord, board[i][j].yCord, board[i][j].xCord, board[i][j].yCord - 2))
                        return false;
                }
