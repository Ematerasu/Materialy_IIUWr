...
(define (expr? e)
  (match e
    ...
    [(apply-expr f xs) (and (lam? f) (list? xs) (map expr? xs))]
    [_ false]))
...
(define (eval-env e env)
  (match e
    ...
    [(apply-expr f xs)
     (if (eq? false (list? (eval xs)))
         (error "ERROR thats not a list")
         (eval-env (apply-app f xs) env))]
    [(lam x e) (clo x e env)]
    [(app f e)
...
;Moja procedura zamieniajaca apply na app
(define (apply-app f xs)
  (match f
    [(lam id e)
     (if (null? (eval (cdr-expr xs)))
         (app f (car-expr xs))
         (if (lam? e)
             (app (apply-app f (cdr-expr xs)) (car-expr xs))
             (app f (car-expr xs))))]
    [_ (error "ERROR applying something else than procedure (lambda)")]))
...
(eval (parse '(apply (lambda (x) (* x x)) (list 3))))
...
(eval (parse '(apply (lambda (x y z) (+ x y)) (if true (list 1 12 3) (list 5 6 7)))))

(eval (parse '(apply (lambda (x y z) (* x (* (* y y) z))) (list 3 4 5))))

;(eval (parse '(apply (lambda (x y) (+ x y)) (list 1 2 3)))) ;Przyklad z errorem
;(eval (parse '(apply + (cons 2 (cons 1 null))))) ;error bo + nie jest lambda
