...
(define (eval-env e env-local env-glob)
  (match e
    [(const n) n]
    [(binop op l r) ((op->proc op) (eval-env l env)
                                   (eval-env r env))]
    [(let-expr x e1 e2)
     (eval-env e2 (env-add x (eval-env e1 env-glob) env-local) env-local)]
    [(letrec-expr x e1 e2)
    ...
    [(lam x e) (clo x e env)]
    [(app f e)
     (let ([vf (eval-env f env-loc)]
           [ve (eval-env e env-loc env-gl)])
       (match vf [(clo x body fun-env)
                  (if (eq? env-empty fun-env)
...
(eval-while (parse-while '((x := 5)
                (f := (lambda (y) (+ x y)))
                (x := 10)
                (z := (f 0)))) env-empty) ;z = 10

(eval-while (parse-while '((x := 5)
                (f := (let [x 50] (lambda (y) (+ x y))))
                (x := 10)
                (z := (f 0)))) env-empty) ;z = 50
