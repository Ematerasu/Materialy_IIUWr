...
(struct symbol-expr (id)       #:transparent)
...
(define (parse q)
  (cond
    ...
    [(eq? q 'null)  (null-expr)]
    [(symbol? q) (var-expr q)]
    [(and (list? q) (eq? (length q) 2) (eq? (first q) 'symbol))
     (symbol-expr (second q))]
...
'(letrec [map (lambda (f lis)
                (if (null? lis)
                    null
                    (cons (f (car lis))
                          (map f (cdr lis)))))])
                          
'(letrec [append (lambda (lis1 lis2)
                   (if (null? lis1)
                       lis2
                       (cons (car lis1)
...
(define test-mlist-1 (mcons 1 (mcons 2 (mcons 3 null))))
(define test-mlist-2 (mcons 1 null))
(define test-mlist-3 null)
...
(define (list->bdlist xs)
    (define (aux  xs prev)
        (if (null? xs)
            null
            (let* ([new-prev (bdlist (car xs) prev null)]
                   [new-next (aux (cdr xs) new-prev)])
                (begin (set-bdlist-next! new-prev new-next)
                    new-prev))))   
  (aux xs null))

(define (cycle-bdlist! xs)
    ( define (aux ys)
        (if (null? (bdlist-next ys))
            (begin (set-bdlist-next! ys xs)
                   (set-bdlist-prev! xs ys))
            (aux (bdlist-next ys))))
(if (null? xs)
    null
    (begin (aux xs) xs)))
