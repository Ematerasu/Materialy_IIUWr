...
(define (eval-env e env)
  (match e
    ...
    [(car-expr e)
     (match e
       [(cons-expr e1 e2) (eval-env e1 env)]
       [_ (eval-env (car (eval-env e env)) env)])]    
    [(cdr-expr e)
     (match e
       [(cons-expr e1 e2) (eval-env e2 env)]
       [_ (eval-env (cdr (eval-env e env)) env)])]
    [(null-expr) null]
...
(eval (parse '(let [x 4] (let [f (lambda (y) (+ x y))] (let [x 0] (f 10))))))
(eval (parse '(car (cdr (cons 1 (cons 2 3))))))
