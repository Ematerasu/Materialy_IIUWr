...
(define init-env (env-add 'number? (prim number?) env-empty))
(define (eval e) (eval-env e init-env-2)); <---!
...
(define init-env-2
  (env-add 'not 
           (clo 'x (if-expr (var-expr 'x) (const false) (const true)) env-empty) 
           env-empty))
