...
(define (concat-map xs f) (map f xs))


(define (flip p x y)
  ((lambda (x) x) (list (cons p x) (cons (- 1 p) y))))

(define (uniform xs)
  (define (propability n ls)
    (if (null? ls)
        null
        (cons (cons n (car ls)) (propability n (cdr ls)))))
  (propability (/ 1 (length xs)) xs))
...
(define (eval-env e env)
  (match e
    ...
    [(if-expr eb et ef)
     ...
     (cons (car vb) (match (cdr vb)
       [#t   (eval-env et env)]
       [#f   (eval-env ef env)]
       [v    (error "Not a boolean:" v)]))))]

    [(uniform-expr xs)
     (uniform (car (eval-env xs env)))]
    
    [(flip-expr p x y)
     (flip (car (eval-env p env))
           (car (eval-env x env))
           (car (eval-env y env)))]
