...
gboolean pobierz_Plansze(gpointer data)
{
    ...
    if (getStringFromPipe(Potoki,wejscie,sizeof(char)*102))
    {
        for(int i=0; i<10; i++)
        {
            for(int j=0; j<10; j++)
            {
                if(wejscie[iterator]=='1')
                {
                    UstawieniePrzeciwnika[i][j]=1;
                }
                else if(wejscie[iterator]=='0')
                {
                    UstawieniePrzeciwnika[i][j]=0;
                }
                iterator++;
            }
        }
        ...
        gtk_label_set_label(GTK_LABEL(StatusGry), "Przeciwnik jest gotowy!");
...
gboolean pobierz_tekst(gpointer data)
{
    ...
    if(getStringFromPipe(Potoki,wejscie,sizeof(char)*100))
    {
        if(strcmp(wejscie, "Zmiana\n")==0)
        {
            Gracz1_move=true;
            ZmianaGrida2();

        }
        if(strcmp(wejscie, "Punkt\n")==0)
        {
            if(strcmp(gracz1,"A")==0) PunktacjaB++;
            else if(strcmp(gracz1,"B")==0) PunktacjaA++;
            ZmianaGrida2();
        }

        if(PunktacjaA==15 || PunktacjaB==15) return false;
    }
